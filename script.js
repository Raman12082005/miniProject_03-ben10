...
// ---------- HERO: build draggable strip ----------
const alienSlider = document.getElementById("alienSlider");
const repeatedAliens = [...heroAliens, ...heroAliens, ...heroAliens];

repeatedAliens.forEach((src, i) => {
  ...
  alienSlider.appendChild(card);
});

// ✅ FIX 1: Calculate drag constraints dynamically AFTER cards are appended
let minTranslate = -(alienSlider.scrollWidth - alienSlider.clientWidth);
const maxTranslate = 0;

// Recalculate on window resize so it never breaks on different screens
window.addEventListener("resize", () => {
  minTranslate = -(alienSlider.scrollWidth - alienSlider.clientWidth);
  // Clamp current position so it doesn't go out of bounds after resize
...
function setSliderPosition() {
  alienSlider.style.transform = `translateX(${currentTranslate}px)`;
}

alienSlider.addEventListener("pointerdown", (e) => {
  isDragging = true;
  startX = e.clientX;
  alienSlider.style.transition = "none";
  alienSlider.setPointerCapture(e.pointerId); // ✅ FIX 2: Prevents losing pointer mid-drag on mobile
});

// ✅ FIX 3: Listen on alienSlider instead of window — more reliable with setPointerCapture
alienSlider.addEventListener("pointermove", (e) => {
...
alienSlider.addEventListener("pointerup", () => {
  if (!isDragging) return;
  isDragging = false;
  prevTranslate = currentTranslate;
  alienSlider.style.transition = "transform 0.3s ease";
});

// ✅ FIX 4: Handle case where pointer leaves the element entirely
alienSlider.addEventListener("pointercancel", () => {
  isDragging = false;
  prevTranslate = currentTranslate;
  alienSlider.style.transition = "transform 0.3s ease";
});
...
const imgEls = sliderImages.querySelectorAll(".alien-img");

function render() {
  sliderEl.style.background = aliens[index].background;
  ...
  h1.textContent = aliens[index].name;
  sliderInfo.appendChild(h1);

  aliens[index].description.split("\n\n").forEach((para) => {
    const p = document.createElement("p");
    p.textContent = para;
    sliderInfo.appendChild(p);
  });

  sliderInfo.classList.remove("animate");
  void sliderInfo.offsetWidth;
  sliderInfo.classList.add("animate");
}

// ✅ FIX 5: Prevent rapid clicking from breaking carousel state
...
// ✅ FIX 6: Keyboard navigation support
